...
const CONFIG = {
  ...
  MAP_HEIGHT: 1400,
  TARGET_UTILITY: 24,    // 이 효용 점수를 먼저 채우는 팀이 나오면 게임 종료(관리자가 조절 가능)
  // 근거(scratchpad/param-search.mjs, reprice-check.mjs 몬테카를로 — 5팀·계획형3+즉흥2 혼합):
  //   기초 15,000 / 집 6,000 고정 / 은행 5,000당 1,000. 게임기·꽃게·소고기·수박 가격을
  //   9,000/8,000/7,000/6,000으로 낮춰서(원래 20,000~30,000이라 "절대 안 사는 물건"이었음)
  //   구매율을 0%에서 20~57%로 끌어올림 — 이 가격 변경으로 진행이 살짝 빨라져 목표점수를 23→24로 올림.
  //   (2026-07-28: 학습지에 원래 이름으로 이미 배포돼서 이름만 원상복구, 가격은 유지)
  //   X=22 → 6턴(30분) / 23 → 7턴 / **24 → 7턴(35분, 7턴내 90%·8턴내 98%)** / 25 → 7~8턴
};
...
// 팀별 효용(1~4점) { [playerId]: { [itemId]: score } } — 물건 16개, 점수마다 4개씩만 줄 수 있음(쿼터제)
let utilities = {};
const SCORES = [1, 2, 3, 4];
const SCORE_LABELS = { 1: '그럭저럭', 2: '좋아요', 3: '너무 좋아요', 4: '꼭 필요해요' };

// 매 턴 시작 시 그 차례의 플레이어에게 가중치(weight) 기반으로 하나 발생 (체력 관련 이벤트는 삭제된 채 유지)
// amount: 돈 변화
const EVENTS = [
...
let gameState = {
  ...
  requiredPlayers: 5,   // 5팀 고정 운영 (테스트용으로 관리자가 1~8 사이로 바꿀 수는 있음)
  round: 1,
...
// 효용 배정 계획: 물품 16개를 1~4점에 4개씩 고르게 나눔
function computeUtilPlan(items) {
  const T = items.length;
  const base = Math.floor(T / 4), rem = T % 4;
  const q = {};
  for (let s = 1; s <= 4; s++) q[s] = base + (s <= rem ? 1 : 0);
  return q;
}
...
function startGame() {
  ...
  if (first) triggerTurnEvent(first.id);
  broadcastState();
}
...
function passTurn() {
  ...
  if (cur && cur.savings > 0) {
    const units = Math.floor(cur.savings / CONFIG.INTEREST_UNIT);
    const interest = units * CONFIG.INTEREST_PER_UNIT;
    if (interest > 0) {
      cur.savings += interest;
      io.emit('notice', `🏦 ${cur.name}: 저축 ${units}묶음(5,000원×${units}) → 이자 +${interest.toLocaleString()}원! (저축 ${cur.savings.toLocaleString()}원)`);
    }
  }
...
io.on('connection', (socket) => {
  ...
  socket.on('act', () => {
    ...
    if (zone.type === 'house') {
      // 집: 두 가지 행동 중 하나를 고르게 함 (선택 후 house:choose에서 결과+턴 종료)
  ...
  socket.on('buy', ({ shopId, itemId }) => {
    ...
    // 한정 수량 없음 — 여러 팀이 같은 물건을 각각 살 수 있음(한 팀당 하나까지)
  ...
  socket.on('save', (amount) => {
    ...
    socket.emit('notice', `${amount.toLocaleString()}원을 저축했어요! (내 차례마다 저축 5,000원당 1,000원씩 이자를 받아요)`);
    broadcastState();
  });
  ...
  socket.on('admin:toUtility', () => {
    ...
    io.emit('notice', '⭐ 물건마다 우리 팀 마음에 드는 정도를 점수로 매겨주세요! (점수마다 4개씩만 줄 수 있어요)');
    broadcastState();
  });
  ...
  socket.on('utility:set', ({ itemId, score }) => {
    ...
    // 이 점수를 이미 몇 개 썼는지 (지금 바꾸려는 물건은 제외) — 점수마다 4개까지만
